#!/usr/bin/env python3
"""
gerar_servers_fogo.py — Retro-Verse / Studios
...
conjunto PODER DE FOGO.

    python3 FERRAMENTAS/preparar_fogo.py      # antes
    python3 FERRAMENTAS/gerar_poses_fogo.py   # antes
    python3 FERRAMENTAS/gerar_servers_fogo.py

DUAS HABILIDADES POR TOOL — M1 + `R`. 14 no conjunto.
...
VFX_MAG = os.path.join(DADOS, "VFXModule_Fogo.lua")
CUTSCENE = os.path.join(DADOS, "CutsceneCam_Fogo.lua")


PREAMBULO = '''-- {objeto}.lua
-- Script de servidor — {tool}  (conjunto PODER DE FOGO)
--
--   M1   {rotulo_m1}
--   R    {rotulo_r}   (a Extra)
--
...
CLIENTE = '''-- Client.lua
-- Script com RunContext = Client — {tool}  (conjunto PODER DE FOGO)
--
...
def main():
    ...
    for tool, d in CONJUNTO.items():
        ...
        total = total + 2
        print("  %-20s %5d linhas · M1 + R%s"
              % (tool, len(corpo.splitlines()), " · CENA" if tem_cena else ""))

    print("")
    print("7 Tool(s), %d habilidade(s) — a queimadura atravessa as sete." % total)
    return 0
