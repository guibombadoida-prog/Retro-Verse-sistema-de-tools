#!/usr/bin/env python3
"""
gerar_poses_xester_v3.py — Retro-Verse / Studios

Escreve o `Poses.lua` das **13 Tools** do Xester — 7 na Forma 1, 6 na Forma 2.

    python3 FERRAMENTAS/gerar_poses_xester_v3.py

CADA TOOL LEVA SÓ AS SEQUÊNCIAS QUE TOCA

    O vocabulário de pose é um só — é o mesmo personagem nas duas formas — mas
    o `Poses.lua` de cada Tool traz apenas as sequências dela, e apenas as
    poses que essas sequências citam.

    Mandar as 24 em todas as 13 seria asset depositado e mudo em 12 delas, e o
    `verificar_beats.py` acusaria "sequência definida e nunca tocada" treze
    vezes por sequência. Uma Tool carrega o que usa.

DOIS VOCABULÁRIOS DE CORPO
...
AS CUTSCENES SÃO SEQUÊNCIAS COMO AS OUTRAS
    ...
    Três Tools levam cutscene: `Eclipse Deck` (a virada, 3.00 s),
    `Curtain Reversal` (a volta, 1.80 s) e `Final Page` (a ultimate, 1.80 s).

O VOCABULÁRIO VEIO DO GERADOR DAS 14

    Já estava conformado e verificado. O arquivo saiu do repositório junto com
    elas, e o conteúdo mora aqui. Só o que o desenho novo pede foi acrescentado:
    as poses de invisibilidade, anel de naipes, castelo, guarda real, sopro
    dracônico, prisma e as cinco da cutscene.
"""
...
# QUEM TOCA O QUÊ
#
# Uma linha por Tool: a forma, e as sequências que o Server dela chama. É a
# mesma lista que o `gerar_servers_xester_v3.py` usa para escrever o corpo —
# as duas saem daqui, então não há como uma tocar o que a outra não escreveu.
# ═══════════════════════════════════════════════════════════════

POR_TOOL = {
    # ── FORMA 1 ───────────────────────────────────────────────────────
    "Xester Curtain Call":       (1, ["CURTAIN_CALL"]),
    "Xester Four Suits Arsenal": (1, ["ARSENAL", "DISPARA_NAIPE"]),
    "Xester Jokers Labyrinth":   (1, ["LABIRINTO"]),
    "Xester Ace Gate":           (1, ["ACE_GATE", "ACE_PORTAO"]),
    "Xester House Collapse":     (1, ["CASTELO", "DESABA"]),
    "Xester Eclipse Deck":       (1, ["ECLIPSE", "TRANSFORMAR"]),
    "Xester Royal Guard":        (1, ["GUARDA_REAL", "REI_LANCA", "REI_BAQUE"]),

    # ── FORMA 2 ───────────────────────────────────────────────────────
    "Xester Wyrm Sparks":        (2, ["WYRM"]),
    "Xester Crown of Cinders":   (2, ["COROA_BRASAS", "BRASAS_CAEM"]),
    "Xester Dragons Requiem":    (2, ["REQUIEM_CARGA", "REQUIEM_SOPRO"]),
    "Xester Prism":              (2, ["PRISMA"]),
    "Xester Final Page":         (2, ["PAGINA_FINAL", "CENA_PAGINA"]),
    "Xester Curtain Reversal":   (2, ["REVERTER"]),
}


CABECALHO = '''-- Poses.lua
-- ModuleScript "Poses" — %(tool)s  (Xester, Forma %(forma)d)
--
-- %(gramatica)s
--
...
--
-- ESTA TOOL TOCA %(quantas)d SEQUÊNCIA(S), e só elas estão aqui. O vocabulário
...
GRAMATICA = {
    1: ("FORMA 1, o BARALHO — gesto de mão, carta e leque. O braço direito\n"
        "-- lidera e o corpo quase não sai do lugar: mágico de salão não se debate."),
    2: ("FORMA 2, o DRAGÃO — o corpo entra inteiro, o HRP lidera o que é peso,\n"
...
def escrever(tool, forma, quais):
    L = [CABECALHO % {"tool": tool, "forma": forma, "quantas": len(quais),
                      "gramatica": GRAMATICA[forma]}]
    L.append("-- AS SEQUÊNCIAS")
    L.append("--")
    for nome in quais:
        natureza, passos = SEQUENCIAS[nome]
...
def main():
    faltando = sorted({n for _f, quais in POR_TOOL.values() for n in quais}
                      - set(SEQUENCIAS))
    if faltando:
        print("sequência pedida e não definida: %s" % ", ".join(faltando))
        return 1

    total_seq = 0
    for tool, (forma, quais) in POR_TOOL.items():
        pasta = os.path.join(TOOLS, tool)
        if not os.path.isdir(pasta):
            print("sem pasta Tools/%s — rode preparar_xester_v3.py antes" % tool)
            return 1
        texto, n_poses = escrever(tool, forma, quais)
        with open(os.path.join(pasta, "Poses.lua"), "w",
                  encoding="utf-8") as f:
            f.write(texto)
        total_seq = total_seq + len(quais)
        print("  %-28s F%d  %d sequência(s) · %2d pose(s)"
              % (tool, forma, len(quais), n_poses))

    sobrando = sorted(set(SEQUENCIAS)
                      - {n for _f, quais in POR_TOOL.values() for n in quais})
    print("")
    print("13 Tool(s), %d sequência(s) escritas." % total_seq)
    if sobrando:
        print("Não usadas por ninguém: %s" % ", ".join(sobrando))
    return 0
