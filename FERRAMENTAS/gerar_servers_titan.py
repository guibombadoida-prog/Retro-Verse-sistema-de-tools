#!/usr/bin/env python3
"""
gerar_servers_titan.py — Retro-Verse / Studios

Escreve o `Server`, o `Client`, o `VFXModule` e o `R6CFrameAnimator` das 7
Tools do conjunto TITAN.

    python3 FERRAMENTAS/preparar_titan.py      # antes
    python3 FERRAMENTAS/gerar_poses_titan.py   # antes
    python3 FERRAMENTAS/gerar_servers_titan.py

TRÊS HABILIDADES POR TOOL — M1 + `R` + `T`

    Sete Tools, 21 habilidades. O `AcaoRemote` é UM: quem separa as duas
    Extras é o nome da tecla no payload, conferido no servidor antes de
    qualquer coisa. Dois remotes seriam duas portas para o mesmo cômodo, e
    duas superfícies para validar.

CONJUNTO AUTORAL

    Não há modelo de origem. Os 21 `SoundId` saem do catálogo do Acervo (é o
    §12.16.2 — "se já existe equivalente, reusar"); a geometria é primitiva
    soldada; a lógica é escrita aqui.

O SOM MORA EM `Tool/SFX/`

    Três por Tool, numa pasta. `somDe(nome)` é o único ponto do Server que
    sabe disso.

O QUE ESTE CONJUNTO NÃO FAZ

    Não escreve `CameraType` em lugar nenhum — o tremor do `VFXModule` é
    multiplicação relativa e some sozinho, então não há câmera a devolver.
    Não usa `ScreenGui`. Não toca em `workspace.Gravity`. E nenhum Server
    move geometria por quadro: onde há trajetória, o cliente desenha a 60 Hz
    e o servidor percorre a MESMA reta por aritmética, só para achar o
    impacto.
"""
...
VFX_TITAN = os.path.join(DADOS, "VFXModule_Titan.lua")


PREAMBULO = '''-- {objeto}.lua
-- Script de servidor — {tool}  (conjunto TITAN)
--
--   M1   {rotulo_m1}
--   R    {rotulo_r}   (Extra 1)
--   T    {rotulo_t}   (Extra 2)
...
CLIENTE = '''-- Client.lua
-- Script com RunContext = Client — {tool}  (conjunto TITAN)
--
...
--
-- Gerado por FERRAMENTAS/gerar_servers_titan.py.
...
local ACAO_R = "Titan_{sufixo}_R"
local ACAO_T = "Titan_{sufixo}_T"
...
from servers_titan import CONJUNTO  # noqa: E402


def escrever(tool, d):
    pasta = os.path.join(TOOLS, tool)
    if not os.path.isdir(pasta):
        print("sem pasta Tools/%s — rode preparar_titan.py antes" % tool)
        return False
    ...
    shutil.copyfile(VFX_TITAN, os.path.join(pasta, "VFXModule.lua"))

    print("%-24s %5d linhas de Server · M1 + R + T"
          % (tool, servidor.count("\n") + 1))
    return True


def main():
    for caminho in (ANIMATOR, VFX_TITAN):
    ...
    for tool, d in CONJUNTO.items():
        if not escrever(tool, d):
            return 1
    print("")
    print("7 Tool(s), 21 habilidade(s) — nenhuma escreve CameraType nem "
          "move geometria por quadro.")
    return 0
