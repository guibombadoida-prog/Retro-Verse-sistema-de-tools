#!/usr/bin/env python3
"""
gerar_servers_jodro.py — Retro-Verse / Studios

Escreve o `Server`, o `Client`, o `VFXModule` e o `R6CFrameAnimator` das 7
Tools do conjunto JODRO.

    python3 FERRAMENTAS/preparar_jodro.py      # antes
    python3 FERRAMENTAS/gerar_poses_jodro.py   # antes
    python3 FERRAMENTAS/gerar_servers_jodro.py

TRÊS HABILIDADES POR TOOL — E É A PRIMEIRA VEZ AQUI

    Todos os conjuntos anteriores são M1 + uma Extra. Estas sete são M1 + DUAS,
    em `R` e `T`.
    ...
    No cliente são dois `BindAction`, e cada um desenha o próprio botão de
    celular (o terceiro argumento do `ContextActionService`). Os dois botões
    ficam em alturas diferentes para não empilhar.
...
VFX_JODRO = os.path.join(DADOS, "VFXModule_Jodro.lua")
...
-- Script de servidor — {tool}  (conjunto JODRO)
--
...
--
-- Gerado por FERRAMENTAS/gerar_servers_jodro.py. Editar aqui à mão faz as sete
...
CLIENTE = '''-- Client.lua
-- Script com RunContext = Client — {tool}  (conjunto JODRO)
--
...
--
-- Gerado por FERRAMENTAS/gerar_servers_jodro.py.
...
local ACAO_R = "Jodro_{sufixo}_R"
local ACAO_T = "Jodro_{sufixo}_T"
...
from servers_jodro import CONJUNTO  # noqa: E402
...
def escrever(tool, d):
    ...
    shutil.copyfile(VFX_JODRO, os.path.join(pasta, "VFXModule.lua"))

    print("%-22s %5d linhas de Server · M1 + R + T"
          % (tool, servidor.count("\n") + 1))
    return True


def main():
    for caminho in (ANIMATOR, VFX_JODRO):
