#!/usr/bin/env python3
"""
gerar_servers_tempo.py — Retro-Verse / Studios

Escreve o `Server`, o `Client`, o `VFXModule`, o `R6CFrameAnimator` e — no
`Fim do Relogio` — a `CutsceneCam` das 7 Tools do conjunto TEMPO.

    python3 FERRAMENTAS/preparar_tempo.py      # antes
    python3 FERRAMENTAS/gerar_poses_tempo.py   # antes
    python3 FERRAMENTAS/gerar_servers_tempo.py
...
O QUE ESTE CONJUNTO CONSERTA DA ORIGEM

    O `timetools.rbxmx` congelava o mundo e NÃO GARANTIA A VOLTA. Ele fazia:

        v.Anchored = true
        wait(script.Stop.Time.Value)
        v.Anchored = false

    Se a Tool fosse destruída, o jogador saísse, ou o script morresse entre as
    duas linhas, a peça ficava ancorada PARA SEMPRE. Num mapa de verdade isso
    é o place inteiro travado, sem nenhuma forma de desfazer.

    Aqui todo estado mexido entra num REGISTRO com o valor de ANTES, e
    `restaurarTudo()` roda no fim do prazo, no `Unequipped` e no `Destroying`.
    É a mesma regra do ragdoll e do efeito de status: o valor de antes é
    guardado e devolvido.

    E o alcance mudou. A origem varria `workspace:GetDescendants()` — o mapa
    inteiro, os outros jogadores, tudo. Aqui é `GetPartBoundsInRadius` num
    raio declarado no CFG.
"""
...
VFX_TEMPO = os.path.join(DADOS, "VFXModule_Tempo.lua")
CUTSCENE = os.path.join(DADOS, "CutsceneCam_Tempo.lua")
...
#: as Tools que NÃO têm portão de hora não declaram `Lighting`. Serviço
#: declarado e não usado é ruído, e num verificador que procura por nome é
#: ruído que vira falso positivo.
SEM_ASTRO = ""
...
-- Script de servidor — {tool}  (conjunto TEMPO)
--
--   M1   {rotulo_m1}
--   R    {rotulo_r}   (Extra 1)
--   T    {rotulo_t}   (Extra 2)
...
CLIENTE = '''-- Client.lua
-- Script com RunContext = Client — {tool}  (conjunto TEMPO)
--
...
-- cliente não replica.
--
-- E A CÂMERA TAMBÉM NÃO. Este arquivo não toca em `workspace.CurrentCamera`
-- em lugar nenhum, e não põe NADA em `game.Lighting` — a origem punha
-- `ColorCorrectionEffect` e `BlurEffect` lá, que é estado do place inteiro.
--
...
local ACAO_R = "Tempo_{sufixo}_R"
local ACAO_T = "Tempo_{sufixo}_T"
...
from servers_tempo import CONJUNTO, COM_CUTSCENE  # noqa: E402
...
def escrever(tool, d):
    ...
    d["bloco_astro"] = BLOCO_ASTRO if d.get("usa_astro") else SEM_ASTRO
    ...
    shutil.copyfile(VFX_TEMPO, os.path.join(pasta, "VFXModule.lua"))
    if tem_cena:
...
def main():
    for caminho in (ANIMATOR, VFX_TEMPO, CUTSCENE):
