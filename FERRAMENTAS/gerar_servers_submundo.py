#!/usr/bin/env python3
"""
gerar_servers_submundo.py — Retro-Verse / Studios

Gera Server, Client, Poses e VFXModule das 6 Tools do conjunto SUBMUNDO.

    python3 FERRAMENTAS/gerar_servers_submundo.py
...
ESQUELETO = '''-- %(arquivo)s
-- Script de servidor — %(titulo)s (conjunto SUBMUNDO)
--
--   M1   %(m1)s
--
...
-- ModuleScript — desenho de efeito, 100%% cliente (conjunto SUBMUNDO)
--
...
\tif not ok then
\t\twarn("[Submundo VFX] pack " .. tostring(nome) .. ": " .. tostring(err))
\tend
\treturn ok
end
...
\tif not ok then
\t\twarn("[Submundo VFX] " .. tostring(tipo) .. ": " .. tostring(erro))
\tend
end
...
def main():
    ...
    print("GERAÇÃO DE SCRIPTS — conjunto SUBMUNDO")
    print("")
