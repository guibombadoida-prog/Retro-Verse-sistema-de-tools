-- IndutordeGravidade_Server_V1.lua
-- Script de servidor — Indutor de Gravidade  (conjunto REALITY)
--
--   M1   abre um poço: a força sobe com a MASSA de cada peça
--
-- ORIGEM: `Gravity Inducer`, de `reality_tools.rbxmx`.
--
--   A origem tinha a ideia certa e duas execuções erradas. A ideia: força proporcional à massa (`massforce = item:GetMass() * force`), que é a única forma de um poço parecer gravidade.
--   Os erros: primeiro, `BodyPosition` — força de mola para um PONTO, então a peça chega, para, e fica colada no centro como um ímã. Poço
--   de verdade acelera, e a coisa PASSA do centro e volta. Segundo, e pior: a singularidade tinha um `Touched` que APAGAVA toda peça solta
--   que a tocasse, e a Tool ainda destruía o membro do alvo que acertasse. As duas coisas estão proibidas aqui, e nenhuma atravessou.
--   E a origem varria `workspace:GetDescendants()` RECURSIVAMENTE a cada 0.25 s por 3.5 s — o place inteiro, quatorze vezes.
--
...
Tool.Equipped:Connect(function()
	...
	rig = Animator.new(personagem, "IndutordeGravidade", Poses,
