# ArmadilhaTemporal — estrutura no Studio

> Conjunto **Guardião do Tempo** · Armadilha plantada no chão
> Origem no modelo: `TemporalTrap (G)`
...
```
Tool  "ArmadilhaTemporal"        CanBeDropped = false · RequiresHandle = true
├── Handle                          Part/MeshPart do relógio (do modelo)
├── DamageClass    StringValue      "Summon"
├── EnergyCost     NumberValue      0
├── RecargaGlobal  NumberValue      5
├── ArmadilhaTemporal_Server_V1 Script
├── Client                          LocalScript
├── VFXRemote                       RemoteEvent  servidor → cliente
├── R6CFrameAnimator                ModuleScript V2 (modo absoluto)
├── Poses                           ModuleScript ← Poses_GuardiaoDoTempo_ArmadilhaTemporal_V1.lua
├── VFXModule                       ModuleScript
├── SFX                             Folder
│   └── Plantar                   Sound  447682521
│   └── Disparo                   Sound  782199941
...
|---|---|---|
| Primária | Armadilha plantada no chão | `Tool.Activated` |
| Extra | — | — |
...
|---|---|---|
| O Server Script, chamando `_G.Combate.recargaGlobal` | `"ArmadilhaTemporal_G"` | **Bloqueia a habilidade.** É a proteção real contra clones na mochila |
| O Núcleo, lendo o `Value` `RecargaGlobal` | `"ArmadilhaTemporal"` (o nome da Tool) | Trava o `Enabled` de todas as cópias — retorno visual |

As duas duram 5s e começam juntas, então não saem de sincronia.

> **Não crie um `ChaveRecarga` com o valor `"ArmadilhaTemporal_G"`.** Isso faria as duas dividirem a
...
1. Criar a `Tool`, nomear `ArmadilhaTemporal`, e pôr o `Handle` (Part/MeshPart do relógio).
