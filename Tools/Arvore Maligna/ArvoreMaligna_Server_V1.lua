...
-- Script de servidor — Arvore Maligna  (conjunto REALITY)
--
--   M1   ergue a árvore, e os cipós AMARRAM quem chega perto
--
-- ORIGEM: `tre`, de `reality_tools.rbxmx`.
--
--   A origem tinha a árvore com um `Touched` que matava, e ela ficava SOLTA — caía, rolava, e às vezes matava o próprio dono ao tombar.
--   Duas coisas foram pedidas explicitamente sobre esta Tool: **ancorar a árvore**, e **não ferir o próprio jogador**. As duas estão
--   cumpridas aqui — o tronco é ancorado e o `ehMinha()` filtra o dono. O que entra de novo é a corda: `RopeConstraint` de verdade, e é a
--   primeira Tool do repositório a usar uma. Quem é amarrado corre, estica, e volta em arco — não desliza em linha reta.
--
...
Tool.Equipped:Connect(function()
	...
	rig = Animator.new(personagem, "ArvoreMaligna", Poses,
