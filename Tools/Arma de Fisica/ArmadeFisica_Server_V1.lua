-- ArmadeFisica_Server_V1.lua
-- Script de servidor — Arma de Fisica  (conjunto REALITY)
--
--   M1   segura a peça no ar; solta com a velocidade que ela tinha
--
-- ORIGEM: `Physics Gun`, de `reality_tools.rbxmx`.
--
--   A origem é a Tool do PRIMEIRO vetor do backdoor — o `qPerfectionWeld` que busca em `assetimport.org` e passa para `require()`. Nada dela atravessou; a mecânica foi lida e reescrita.
--   E a mecânica valia a leitura: `BodyPosition` na peça mirada, mantida a uma distância do cano, seguindo o mouse. O problema é que o
--   `maxForce` dela era `math.huge*math.huge` — força infinita, que arrasta um prédio com a mesma facilidade que um cubo. Nada tinha peso.
--   Aqui é `AlignPosition` com `MaxForce ∝ massa` e `Responsiveness` que CAI com a massa: a peça pesada chega atrasada, balança, e bate nas
--   coisas no caminho. E ao soltar ela sai com a velocidade que já tinha, que é o que transforma a garra numa arma.
--
...
Tool.Equipped:Connect(function()
	...
	rig = Animator.new(personagem, "ArmadeFisica", Poses,
