-- CutsceneCam.lua
-- Script com RunContext = Client — conjunto CRIAÇÃO
--
-- Vive SÓ no `Demiurgo`. É a ultimate do conjunto, e a única com recarga alta
-- o bastante (44 s) para uma cena não virar tempo morto.
--
--═══════════════════════════════════════════════════════════════
...
--
--═══════════════════════════════════════════════════════════════
-- O QUE MUDA NUMA CENA DE CRIAÇÃO
--═══════════════════════════════════════════════════════════════
--
--   Numa cena de bomba a câmera ABRE no impacto: o FOV estoura porque a coisa
--   se espalha. Aqui é o contrário, e é o que dá identidade à cena: o que
--   acontece é o chão SUBINDO, e a câmera tem de subir com ele.
--
--   Por isso os offsets do `INVOCADOR` ganham altura a cada beat — 16, 22,
--   34 — enquanto o FOV vai FECHANDO até o `ESTOURA`, e só então abre. É a
--   diferença entre filmar uma explosão e filmar uma construção.
--
--   E o ancoradouro do INVOCADOR é o PONTO, não o portador: o que interessa
--   enquadrar é o que está sendo criado.
--
--═══════════════════════════════════════════════════════════════
-- AS SEIS REGRAS DA GRAMATICA_CUTSCENE
--═══════════════════════════════════════════════════════════════
--
--   1. FOV é a técnica principal. Fecha até 44 na carga — o mais fechado do
--      repositório — e abre para 98 no fim. Fechar mais do que os outros é o
--      que faz o mundo parecer grande antes de ele existir.
--   2. Enquadramento POR ESPECTADOR: quem criou vê de fora e de cima; quem
--      está no raio vê o chão nascendo debaixo dele, de baixo para cima.
--   3. Aproximação EXPONENCIAL, nunca Tween bloqueante.
--   4. Estágio troca por TEMPO dentro do RenderStepped.
--   5. Tremor com ENVELOPE, e só na janela do assentamento.
...
local QUADROS = {
	INVOCADOR = {
		...
		CENA    = { de = Vector3.new(15, 16, 24), olhar = "ponto", fov = 62, tremor = false },
		-- fecha: o FOV é a técnica (regra 1)
		CARGA   = { de = Vector3.new(11, 22, 18), olhar = "ponto", fov = 44, tremor = false },
		-- e ESTOURA — abre tudo e sacode
		ESTOURA = { de = Vector3.new(24, 34, 32), olhar = "ponto", fov = 98, tremor = true },
		-- FIM assenta antes de devolver. Sem ele a câmera volta no mesmo quadro
		-- do impacto, e a cena termina sem respirar.
		FIM     = { de = Vector3.new(20, 28, 30), olhar = "ponto", fov = 74, tremor = false },
	},
	ALVO = {
		-- a metade da regra 2 que é do alvo: ele ancora em SI MESMO e vê a
		-- coisa chegando nele, não o outro trabalhando
		CENA    = { de = Vector3.new(-7, 2.6, 13), olhar = "eu",    fov = 68, tremor = false },
		CARGA   = { de = Vector3.new(-4, 1.8, 8),  olhar = "eu",    fov = 52, tremor = false },
		ESTOURA = { de = Vector3.new(-9, 3.4, 15), olhar = "eu",    fov = 100, tremor = true },
