-- GatoAjudanteBoss_Server_V1.lua
-- Script de servidor — Gato Ajudante Boss  (conjunto REALITY)
--
--   M1   invoca o gato, que persegue POR FÍSICA até o prazo
--
-- ORIGEM: `gravity cat not amused`, de `reality_tools.rbxmx`.
--
--   A origem era um spawner de NPC: `Humanoid`, seis `Motor6D`, script de `Animate` de 483 linhas e uma IA própria. O `CLAUDE.md` põe NPC fora de escopo, e com razão — manter sistema de NPC não é habilidade de Tool.
--   O que fica é a INVOCAÇÃO, e ela ganha o que a origem não tinha: o gato é um corpo FÍSICO. Ele não teleporta atrás do alvo por CFrame; ele
--   é puxado por `AlignPosition`, então ele colide, atrasa nas curvas, bate nas coisas e pode ser empurrado. E é isto que resolve a proibição
--   mais dura do repositório: **servidor não move geometria por quadro**. Quem move o gato é o solver, e a replicação sai de graça.
--
...
Tool.Equipped:Connect(function()
	...
	rig = Animator.new(personagem, "GatoAjudanteBoss", Poses,
