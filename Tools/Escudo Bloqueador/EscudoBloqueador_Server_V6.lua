...
--   [MIG] A redução passa a ser registrada em _G.Combate.registrarReducao
...
local function ligarPassivo()
	...
	if _G.Combate and _G.Combate.registrarReducao then
		local cancelar = _G.Combate.registrarReducao(character, CFG.REDUCAO_PORTADOR)
		guardarCancelamento(cancelar)
	end
	...
	guardarConexao(humanoid.HealthChanged:Connect(function(nova)
		...
		if not (_G.Combate and _G.Combate.registrarReducao) then
			corrigindoVida = true
