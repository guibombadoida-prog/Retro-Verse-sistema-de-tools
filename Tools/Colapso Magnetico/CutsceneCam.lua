-- CutsceneCam.lua
-- Script com RunContext = Client — Colapso Magnetico  (conjunto MAGNETISMO)
--
-- Vive SÓ no `Colapso Magnetico`. É a ultimate do conjunto, e a única com
-- recarga alta o bastante (46 s) para uma cena não virar tempo morto.
--
--═══════════════════════════════════════════════════════════════
...
--
--═══════════════════════════════════════════════════════════════
-- O QUE MUDA NUMA CENA DE MAGNETISMO
--═══════════════════════════════════════════════════════════════
--
--   Numa cena de criação a câmera SOBE, porque o chão sobe. Aqui ela CAI PARA
--   DENTRO, porque é o que a força faz: o colapso puxa tudo para um ponto, e
--   a câmera é a última coisa a ser puxada.
--
--   Os offsets do INVOCADOR ENCOLHEM a cada beat — 30, 20, 9 — enquanto o FOV
--   também fecha. Os dois juntos são o que dá a sensação de sucção: aproximar
--   e estreitar ao mesmo tempo é o oposto do dolly-zoom, e é a leitura de
--   "está me puxando".
--
--   E o ancoradouro do INVOCADOR é o PONTO, não o portador: o que interessa
--   enquadrar é o poço, não quem o abriu.
--
--═══════════════════════════════════════════════════════════════
-- AS SEIS REGRAS DA GRAMATICA_CUTSCENE
--═══════════════════════════════════════════════════════════════
--
--   1. FOV é a técnica principal, e aqui ele FECHA no clímax em vez de abrir.
--   2. Enquadramento POR ESPECTADOR: quem abriu vê o poço de fora; quem está
--      no raio vê de dentro, com o poço acima da linha dos olhos.
--   3. Aproximação EXPONENCIAL, nunca Tween bloqueante.
--   4. Estágio troca por TEMPO dentro do RenderStepped.
--   5. Tremor com ENVELOPE, e só na janela da singularidade.
...
--
-- Os QUATRO beats são exatamente as quatro marcas da sequência `EXTRA_T` do
-- `Poses.lua` — CENA_ABRE, CENA_CARGA, SINGULARIDADE, CENA_FIM. É de propósito: beat que a câmera
...
local QUADROS = {
	INVOCADOR = {
		-- longe e alto: o poço inteiro no quadro, ainda calmo
		CENA_ABRE = { de = Vector3.new(18, 15, 26), olhar = "ponto",
			fov = 68, tremor = false },
		-- e começa a CAIR PARA DENTRO. Os dois ao mesmo tempo — aproximar e
		-- estreitar — são o oposto do dolly-zoom, e é a leitura de sucção.
		CENA_CARGA = { de = Vector3.new(12, 10, 17), olhar = "ponto",
			fov = 54, tremor = false },
		-- o clímax FECHA em vez de abrir. É a inversão da gramática do
		-- repositório, e é o conjunto inteiro num quadro: numa explosão o
		-- mundo se afasta; num colapso ele é engolido.
		SINGULARIDADE = { de = Vector3.new(5, 4.5, 8), olhar = "ponto",
			fov = 38, tremor = true },
		-- FIM assenta antes de devolver. Sem ele a câmera volta no mesmo
		-- quadro do colapso, e a cena termina sem respirar.
		CENA_FIM = { de = Vector3.new(14, 12, 20), olhar = "ponto",
			fov = 72, tremor = false },
	},
	ALVO = {
		-- a metade da regra 2 que é do alvo: ele ancora em SI MESMO, e o poço
		-- fica ACIMA da linha dos olhos dele — quem está sendo puxado olha
		-- para cima, não de igual para igual
		CENA_ABRE = { de = Vector3.new(-6, 2.4, 12), olhar = "eu",
			fov = 70, tremor = false },
		CENA_CARGA = { de = Vector3.new(-4, 1.4, 8), olhar = "eu",
			fov = 56, tremor = false },
		SINGULARIDADE = { de = Vector3.new(-2.5, 0.8, 5), olhar = "eu",
			fov = 40, tremor = true },
		CENA_FIM = { de = Vector3.new(-7, 3.2, 13), olhar = "eu",
			fov = 72, tremor = false },
	},
}
...
CutsceneRemote.OnClientEvent:Connect(function(evento, dados)
	...
	if evento == "INICIO" then
		...
		quadroAtual = QUADROS[papel][dados.nome or "CENA_ABRE"]
			or QUADROS[papel].CENA_ABRE
		posAtual, focoAtual = nil, nil
		desdeBeat = 0
		seguir()
		pular()
